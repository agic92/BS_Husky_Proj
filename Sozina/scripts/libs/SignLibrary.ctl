...
//return group that sign pictogram belongs
string getGroupSign(int pictureId)
{
  dyn_string getConfigFromDp, splitArray;
  mapping map;
  dpGet($SYSTEMNAME + "_mp_SignSettings.settings.signGroupConfiguration", getConfigFromDp);
  if(dynlen(getConfigFromDp) == 0)
  {
      Log::error("getGroupSign", "DP is empty" + getConfigFromDp);
      return "Error";
  }
  ...
  for(int i=1; i <= dynlen(getConfigFromDp); i++)
  {
     ...
     {
        Log::error("getGroupSign", "splitArray is < 2" + splitArray);
        return "Error";
     }
     map[splitArray[1]] = splitArray[2];
  }
  return map[(string)pictureId];
}
...
string getItemGroup(string signGroupId)
{
  dyn_string signGroups, splitArray;
  mapping map;
  dpGet($SYSTEMNAME + "_mp_SignSettings.settings.signGroups", signGroups);
  if(dynlen(signGroups) == 0)
  {
      Log::error("getItemGroup", "DP is empty" + signGroups);
      return "Error";
  }

  for(int i=1; i <= dynlen(signGroups); i++)
  {
     splitArray = strsplit(signGroups[i], "|");
     if(dynlen(splitArray) < 2)
     {
        Log::error("getItemGroup", "splitArray is < 2" + splitArray);
        return "Error";
     }
     map[splitArray[1]] = splitArray[2];
  }
  return map[signGroupId];
}
