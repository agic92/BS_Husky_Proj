  ...
  // uzima samo zadnji iz liste zato sto u nekim slucajevima posalje vise alerta za istu promjenu jednog DP-a sto pravi lazne alerte
    ...
    if(visible)
    {
       ...
       if(bState)
      {
        ...
        if(bFire)
          set_camera_on_first_empty_Alarm_Monitor_for_Fire(sAlert_Camera_Ip, lAlarmMonitors);
        else if(bWrongWay)
    ...
    {      
      bool bWrongWay = is_wrongWay(sDPE);
      bool bFire = is_Fire(sDPE);
      alert_cleared(lAlarmMonitors, sAlert_Camera_Ip, sSys_DP, bWrongWay, bFire);
    }
...
void set_camera_on_first_empty_Alarm_Monitor_for_Fire(string sCamera_IP, dyn_dyn_anytype lAlarmMonitors)
{
  int i;     
  bool bFound_empty_AlarmMonitor = false;
  //ovdje se moze dodati umjesto donje petlje upit u bazu da vrati AlarmMonitore koji imaju camareIP == 0
...
void alert_cleared(dyn_dyn_anytype alarmMonitorList, string sIP, string sAlert_Sys_DP, bool bWrongWay, bool bFire)
{
...
void set_camera_for_Fire(string sAlert_Camera_Ip, dyn_dyn_anytype lAlarmMonitors) 
{
  set_AlarmMonitorDP_by_name(sAlarmMonitorName, sIP, true);
