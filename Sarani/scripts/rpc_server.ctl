...
mixed methodHandlerOwn(string sMethod, dyn_mixed &asValues, string user, string cookie)
{
  ...
  switch (sMethod)
  {
    ...
    case "setDP" :
        ...
        //rc=dpSet(makeDynString(asValues[1]), makeDynInt(asValues[2])); 
        //rc=dpSet(makeDynString(asValues[1]), makeDynAnytype(asValues[2])); 
        //rc=dpSet(makeDynString(asValues[1], asValues[3]), makeDynInt(asValues[2], asValues[4])); 
