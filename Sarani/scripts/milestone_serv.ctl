...
//-dbg 1 - prikaz promjena dataPont-a vezanih za Milestone
//-dbg 2 - prikaz javljanja client-a za Milestone
...
mixed methodHandlerOwn(string sMethod, dyn_mixed &asValues, string user, string cookie)
{
  ...
  switch (sMethod)
  {
    ...
    case "getDP":
        DebugFTN("level1", "Get data point " + asValues[1]);
        int x;
        rc = dpGet(asValues[1], x);
        DebugFTN("level1", rc, x);
        if(rc == 0)
          return x;
        else
          return -1;        
