/*******************************************************************************
  ...
  Note:
    ...
    Alarm: 0 - none, 1 - wrong direction, 2 - loop fault, 3 - traffic jam,
           4 - bumper to bumper, 5 - lost connection.
********************************************************************************/

#uses "CtrlXmlRpc"
#uses "loop_detector_ini.ctl" // devices.

mapping settings;
int globalLock = 0;
dyn_int locks;
dyn_bool isPrimaryServer;
dyn_bool isActive;
dyn_mapping lastVehicles;
mapping alarms;
mapping scriptBlocked;
mapping threads;
mapping connectionAlarm;
time lastLogTime = 0;
bool activationDelay = 2;
...
int main()
{
  ...
  alarms = makeMapping(
    ...
    3, "traffic jam", 4, "bumper to bumper", 5, "lost connection");
  ...
  locks = makeDynInt(); isPrimaryServer = makeDynBool();
  xmlrpcClient();
  
  int count = settings["DevicesCount"];
  for (int i = 1; i <= count; i++) {
    threads[i] = -1; isPrimaryServer[i] = getIsPrimaryServer(i); isActive[i] = true;
    connectionAlarm[i] = makeDynInt(-1, -1);
    startDeviceThread(i); }
...
bool checkResult(int id, string method, dyn_mixed args, mapping result)
{
  if (mappinglen(result) == 0) {
    writeWarning(formatString("checkResult (device: %s, method: %s, args: {%s})",
      devices[id], method, args), "Server returned empty result.");
    return false; }
  else {
    writeTrace(formatString("checkResult (device: %s, method: %s, args: {%s})",
      devices[id], method, args), "Server returned valid data.");
    return true; }
}

bool getIsPrimaryServer(int id)
{
  /* citanje konfiguracije na adapteru (obsolete)
  try {
    if (rpcConnect(id)) {
      dyn_mixed params; bool result;
      if (rpcCall(id, "LoopDetectorProtocolAdapter.isPrimaryServer", params, result))
        return result; }
    else writeWarning("isServerActive", "Failed to connect to the server."); }
  catch { exceptionHandler(); }
  return true;
  */
  return myReduHostNum() == 1;
}

bool isServerActive(int id)
{
  string dp = (isPrimaryServer[id] ? "_ReduManager" : "_ReduManager_2") + ".Status.Active";
  return dpGetOrDefault(dp, false);
}

void setConnectionTimestamp(int id)
{
  string dp = (isPrimaryServer[id] ? "BrojacPrometa" : "BrojacPrometa_2") + ".IntegrClient.timestamp";
...
void threadProc(int id)
{
  ...
  while (true) {
    ...
    bool serverActive = isServerActive(id);
    if (serverActive) {
      if (!isActive[id]) {
        isActive[id] = true;
        writeInfo("threadProc", formatString(
          "Server is activated for device [%s].", devices[id]));
      ...
      if (isActive[id]) {
        isActive[id] = false;
        writeInfo("threadProc", formatString(
          "Server is deactivated for device [%s].", devices[id])); }
    
    setScriptBlocked(id, false); setConnectionTimestamp(id); delay(settings["LoopDelay"]); }
}
...
bool getVehicleData(int id, mapping &data)
{
  ...
  try {
    ...
    bool module1Alarm = 1, module2Alarm = 1;
    if (rpcConnect(id)) {
    
      string device = devices[id];
      int address = dpGetOrDefault(device + ".DeviceBusAddress", 50);
      mapping serverSettings = getServerSettings(id), result;
      string method = "getVehicleData"; dyn_mixed params;
  
      params = makeDynMixed(serverSettings, 1, address, 0x1F);
      if (rpcCall(id, "LoopDetectorProtocolAdapter." + method, params, result)) {
        data["Module1"] = result;
        if (checkResult(id, method, params, result)) module1Alarm = 0; }
  
      params = makeDynMixed(serverSettings, 2, address, 0x1F);
      if (rpcCall(id, "LoopDetectorProtocolAdapter." + method, params, result)) {
        data["Module2"] = result;
        if (checkResult(id, method, params, result)) module2Alarm = 0; }}
	  
    else writeWarning("getVehicleData", formatString(
      "Failed to connect to the server for device [%s].", devices[id]));
    
    if (connectionAlarm[id][1] != module1Alarm) {
      storeAlarm(id, 0, 5, module1Alarm); connectionAlarm[id][1] = module1Alarm; }
    if (connectionAlarm[id][2] != module2Alarm) {
      storeAlarm(id, 1, 5, module2Alarm); connectionAlarm[id][2] = module2Alarm; }}
...
bool getVehicleCounter(int id, mapping &data)
{
  ...
  try {
    ...
    if (rpcConnect(id)) {
      ...
      int address = dpGetOrDefault(device + ".DeviceBusAddress", 50);
      mapping serverSettings = getServerSettings(id), result;
      string method = "getVehicleCounter"; dyn_mixed params;
  
      params = makeDynMixed(serverSettings, 1, address, 0x0F);
      if (rpcCall(id, "LoopDetectorProtocolAdapter.getVehicleCounter", params, result)) {
        data["Module1"] = result;
        if (checkResult(id, method, params, result)) module1Alarm = 0; }
  
      params = makeDynMixed(serverSettings, 2, address, 0x0F);
      if (rpcCall(id, "LoopDetectorProtocolAdapter.getVehicleCounter", params, result)) {
        data["Module2"] = result;
        if (checkResult(id, method, params, result)) module2Alarm = 0; }}
...
int getAlarmFromVehicleData(int id, string module, mapping &data)
{
  ...
  string key; int class_ = -1, direction = -1, length = -1, speed = -1;
  if (mappingHasKey(data, key = "VehicleClass")) class_ = data[key];
  ...
  if (class_ == 0 && length == 40 && speed == 0) return 2;
  if (class_ == 0 && length == 40 && speed == 5) return 3;
  if (class_ == 0 && length == 45 && speed == lastVehicles[id]["VehicleSpeed"]) return 4;
...
// set: -1 - set and clear, 0 - clear, 1 - set.
bool storeAlarm(int id, int module, int alarm, int set)
{
  writeTrace("storeAlarm", formatString(
    "id: %s, module: %s, alarm: %s, set: %s", id, module, alarm, set));
  ...
  if (set != 0 && dpSet(dp, alarm) != 0) { errorHandler(); writeWarning(method, dp); return false; }
  if ((set == -1 || (set == 0 && dpGetOrDefault(dp, alarm) == alarm)) && dpSet(dp, 0) != 0) {
    errorHandler(); writeWarning(method, dp); return false; }
  
  string operation = set == -1 ? "triggered" : set == 0 ? "cleared" : "set";
  writeInfo("storeAlarm", formatString(
    "Alarm [%s] %s for device [%s], module [%s].", alarms[alarm], operation, dpDevice, dpModule));
...
bool setScriptBlocked(int id, bool blocked)
{
  ...
  string method = "setScriptBlocked";
  
  bool alarm = dpGetOrDefault(dp, !blocked);
  if (blocked != alarm) {
    if (dpSet(dp, blocked) != 0) { 
      errorHandler(); 
      writeWarning(method, dp); 
      return false; }
