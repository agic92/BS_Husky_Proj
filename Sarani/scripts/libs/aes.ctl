...
const string AES_LANG_DE      ="de_AT.iso88591";
const string AES_LANG_EN      ="en_US.iso88591";
const string AES_LANG_META    ="en_US.iso88591";
...
global dyn_string g_dsWorkAttr, g_dsWorkName, g_dsWorkVsbl;
global dyn_dyn_anytype g_ddaSavedConfigMatrix, g_ddaTempConfigMatrix;

void doTimeStamp( const string text, bool init=false )
{
  return;
  const string _func_="doTimeStamp";
  const int d=5;
  
  string tmp;
  float startDiff=0.0, lastDiff=0.0, currTime;


  currTime=getCurrentTime();

  if( init || ( g_startTime == 0 ) )
  {
  ...
  {
    g_counterTime++;
    startDiff=currTime - g_startTime;
    lastDiff=currTime - g_lastTime;
    // save current time
    g_lastTime=currTime;
  }

//  sprintf( tmp, "TIMESTAMP(%03d)[%30s] : startdiff=%6.2f stepdiff=%6.2f start=%13.2f curr=%13.2f", g_counterTime, text, startDiff, lastDiff, g_startTime, currTime );

  tmp="TIMESTAMP(" + g_counterTime + ")[" + text + "] : ";
  tmp+="startdiff=" + startDiff + " ";
  tmp+="stepdiff=" + lastDiff + " ";
  tmp+="start=" + g_startTime;
  tmp+="curr=" + currTime;
...
/**
  Main routine to process the column click event. 
  @param none
...
int aes_checkForAddValues(string dpe, time alertTime, int count)
{
  ...
  {
    dyn_anytype daAllValues;
    alertGet(alertTime, count, dpSubStr(dpe, DPSUB_SYS_DP_EL_CONF_DET) + "._add_values", daAllValues);
    if (dynlen(daAllValues))
    {
      return dynlen(daAllValues);
    }
  }
...
{
  const string _func_="aes_initTable";
  const int d=5;
  string oldColName;
  shape tableShape;
  dyn_dyn_anytype dda;
...
//  if( mode != AES_MODE_CURRENT )
  ...
  for( n=1; n<=AES_MAX_COLUMNS; n++ )
  {
    ...
    {
       colVis  =dda[n][CHK_VISIBLE];
       tableShape.columnVisibility(n)=colVis;
    }
    else
    {
...
{
  const string _func_="aes_createAttributes() / ";
...
{
  const string _func_="aes_doColumnDblClick";
  const int d=0;
  int screenType;
  int colIdx;
  ...
  {
    aes_debug( _func_+"() / Unkown function name="+funcName+" !" );
  }
...
int aes_getDpidFromTable( const int row, string &dpid, mapping mTableRow )
{
  ...
  aes_debug( _func_+"() / row="+row+" full="+fullName+" substr="+dpid , d );
...
{
  // uses the panelgolbal propdpnames
...
{
  const string _func_="aes_doColumnClick";
  int screenType;
  int colIdx;
  string funcName;
  string propDpName;
...
/*
  min=FIRST_TABLE+offset;
  max=LAST_TABLE+offset;

  if( objIdx < min || objIdx > max )
  {
    aes_debug("aes_getTableSettingsValue - objIdx="+objIdx+" not found!");
    return (int)0;
  }
*/

  //da=vst[objIdx+offset];
...
string aes_getModeStrLang( int mode )
{
  ...
  const string _func_="aes_getModeStrLang";
  const int d=0;
  ...
  {
    tmp="Unkown mode " + mode + " !";
    ret="";
  }

  aes_debug( _func_ + "() / mode(type)=" + mode + " text=" + ret, d );
...
{
  const string _func_="aes_doPropertyChild";
  const int d=0;
  unsigned screenType;
...
//aes_debug( _func_+"() >>>>>>>>>>>>>>>>>>>>>>> colNames=" + colNames, 10 );
//aes_debug( _func_+"() >>>>>>>>>>>>>>>>>>>>>>> colTitles=" + colTitles, 10 );
  ...
  // runmode muss vor dem dialog ( der ggf properties setzt ) auf unlgeich running gesetzt werden damit hotlink in properties nicht automatisch ausgef�hrt wird
...
aes_debug(_func_+"panelName="+panelName, d );
aes_debug(_func_+"tabTypeStr="+tabTypeStr, d );
aes_debug(_func_+"screenTypeStr="+screenTypeStr, d );
aes_debug(_func_+"configName="+configName, d );
aes_debug(_func_+"propDp="+propDp, d );
aes_debug(_func_+"balPropDp="+balPropDp, d );
...
{
  const string _func_="aes_getTableShape";
  ...
  {
    aes_debug( _func_+"() / Unkown tabType="+tabType+" !" );
    return -1;
  }
...
void aes_appendColumnObject( int screenType, dyn_dyn_anytype &dda )
{
  ...
  const string _func_="aes_appendColumnObject";
  ...
  //aes_debug( _func_+"() / ==========> ddaRes(" + screenType + ")=" + dynlen( ddaRes[screenType] ), 10 );

}


/**
  This function returns attributs ( dyn_string ) for a given "onClick" function name. 
  @param funcName Function name
  @author Martin Pallesch
  @version 1.0
  @return nothing
...
//  if ( funcName == AECSF_ACKNOWLEDGE )
  ...
  {
    aes_debug(_func_+"() / Unkown funcname=" + funcName );
    return -1;
  }
...
/**
  This function returns attributs ( dyn_string ) for a given function name. 
  @param funcName Function name
  @author Martin Pallesch
  @version 1.0
  @return nothing
...
{
  const string _func_="aes_getAttributes4FuncName";
  const string al="_alert_hdl..";
  const string of="_offline..";
...
//******************************************
  ...
  if ( funcName == AECSF_WRITEFSTR )
  {
    //attrib=makeDynString(  );
  }

  else if ( funcName == AECSF_STATUSBIT )
    attrib=makeDynString( of+"_status" );             // events

  else if ( funcName == AECSF_DPGETACOUNT )
    attrib=makeDynString( _TIME_ );

  else if ( funcName == AECSF_DPGETDESC )
    attrib=makeDynString( _DPID_ );
  ...
  else if ( funcName == AECSF_DPGETCONTENT )
    attrib=makeDynString( _DPID_, of+"_text", of+"_value" );  // events

  else if ( funcName == AECSF_DIRECTION )
    attrib=makeDynString( al+"_direction" );                  // alerts

  else if ( funcName == AECSF_DPFORMATVAL )
  {
    if( screenType == AESTYPE_EVENTS )
    {
      attrib=makeDynString( _DPID_ , of+"_value" );
    }
    else
    {
      attrib=makeDynString( _DPID_ , al+"_value" );
    }  
  }
  else if ( funcName == AECSF_DPSUBSTR )
    attrib=makeDynString( _DPID_ );

  else if ( funcName == AECSF_GETEXTTIMEID )
    attrib=makeDynString( _TIME_ );

  // new Halper - only events
  else if ( funcName == AECSF_USERNAME )
    attrib=makeDynString( of + "_user" );

  // new Halper - only events
  else if ( funcName == AECSF_MAN2NAME )
    attrib=makeDynString( of + "_manager" );

  // new Halper - only events
  else if ( funcName == AECSF_MAN2SHORTNAME )
    attrib=makeDynString( of + "_manager" );

  else
  {
    aes_debug("aec_getAttrIdx4FuncName() / Unkown funcname=" + funcName );
    return -1;
  }

aes_debug(_func_+" attribute names="+attrib );
...
//void aes_setTableProperties( const dyn_dyn_anytype &ddaCol )
{
...
/**
  This function returns the index for a given name within the uniq attribute list. 
  @param uniqAttr
  @param name
  @author Martin Pallesch
  @version 1.0
  @return Index of the search attribute name
*/
/*
int aes_getUniqAttrIdx( const dyn_string &uniqAttr, string &name )
{
  int l,n;
  bool found=false;
  ...
  if( found )
  {
    // important - ATTR_OFFSET for _dpid_ and _time_ ( == 2 )
    //return ( n + ATTR_OFFSET );
    return n;
  }
  else
  {
    aes_debug( "aes_getUniqAttrIdx - Attribute:"+name+" not found !" );
    return -1;
  }
...
{
  const string _func_="aes_message";
  const int d=0;
...
aes_debug( _func_+"() id=" + id + " mode=" + mode, d );
  ...
  switch( id )
  {
    ...
    */
    ...
    default:
      ///////// throwError !!!!!!
      aes_debug( _func_ + "() / Unkown message id=" + id + " !", d );
      return;
  }
...
{
  const string _func_="aes_getMessageTexts";
  const int d=5;
  dyn_string ds;
  ...
  {
    aes_debug( _func_ + "() / No keys found for id=" + id + " !" );
    return -1;
  }
...
{
  const string _func_="aes_getCatStr";
...
{
  const string _func_="aes_getAttrIndexList";
  ...
  {
    aes_debug( _func_+"() / Unexpected valuetype="+valueType+" !" );
  }
  ...
  if( l > 0 )
  {
    for( n=1; n <= l; n++ )
    {
      ...
      if( pos <= 0 )
      {
        aes_debug(_func_+"() / No index for attribute="+workAttr[n]+" found !" );
      }
      else
      {
        dynAppend( diRes, pos );
  ...
  //aes_debug(_func_+"() / Function="+fu+" attrIdx="+diRes );
...
/**
  ...
  @return nothing
*/
//string aes_getAttributeString( const int screenType, const dyn_string &uniqAttr )
string aes_getAttributeString( const dyn_string &uniqAttr, int screenType, int valueType, int valTypeAlertSummary )
{
  const string _func_="aes_getAttributeString";
  int l, n;
  string res="";
  string komma="";
  string attr;
...
{
  const string _func_="aes_getLastError";
  int code;
  if( dynlen( err ) )
  {
...
{
  const string _func_="aes_debugConfigMatrix";
  const int d=4;
  ...
  if( l <= 0 )
  {
    aes_debug(_func_+"() / Empty config matrix !", 1 );
    return;
  }
...
{
  const string _func_="aes_debugAttributes";
  const string ddot="..";
...
dyn_string aes_correctTimeSort( dyn_string dsSort )
{
  ...
  const string _func_="aes_correctTimeSort";
  dyn_string dsResult;
  int pos=dynContains( dsSort, _TIME_STR_ );

  //DebugN(_func_+"() / before sortList="+dsSort );
  ...
  //DebugN(_func_+"() / after sortList=" + dsResult );
...
{
  const string _func_="aes_getRealIdent";
  int sysId;
  string sAnswerSystem="";

/*
DebugN(_func_+" tab=" + dynlen(tab) );
DebugN(_func_+" tab=" + tab );
DebugN(_func_+" err=" + dynlen(err) );
DebugN(_func_+" err=" + err);
*/
...
bool aes_discardingTreating( dyn_errClass err, string ident, bool fromDpQuery=false )
{
  ...
  const string _func_="aes_discardingTreating";
...
/**
  ...
  @return nothing
*/
bool aes_reduDistTreating( dyn_errClass err, int systemId, bool fromProp=false )
{

  const string _func_="aes_reduDistTreating";
  const int d=5;
  string sysName, pureSysName;
  int errorCode;
  ...
  {
    errorCode=getErrorCode( err[1] );   ////////�������������������������������������� TODO: letzten verwenden laut Theis
  }


  // nur system verloren - dist
  if( errorCode == AES_ERR_DISTCONLOST )
  {
    // extrahiere system aus errorcode / weil keine HL Daten zum extrahieren
    // Pruefe - param sysId muss DUMMY ID SEIN !!!!!!!!!!!!!!!!!!
    //string ern=getErrorDpName( err[1] );
    //systemId=getSystemdId( ern );             ////////// CHRISTOPH FRAGEN !!!!!!!

    // sysid sollte schon stimmen - siehe getRealIdent
    aes_removeIdent4SysId( systemId );
    // l�sche sys aus connect liste
    // return to keep last information before connection lost
  ...
  // hier und nicht in refresh damit im offenen mode distinfo bei ersten daten refreshed werden
  aes_createDistInfo();
  return true;
}
...
{
  const string _func_="aes_createDistInfo";
  const int d=5;
  int sysToConnect, connectedSys;
  dyn_string systems, params, ds, ds1;
  dyn_uint du;
  string pre, app, backCol;
  string checkAll, val, valHidden;
  int i, l, tabType, mode, sysId;
  int ic=1, closedSys;
  bool fromProp;
  dyn_string dsTmp;
  
  //aes_debug( _func_+"() / g_checkAll=" + g_checkAll + " g_systemSelections=" + g_systemSelections + " g_CounterConnectId=" + g_counterConnectId, d );

  closedSys=dynlen( g_closedIdf );

  if( g_checkAll )
  {
    int ret = getSystemNamesConnected(systems, du);
    sysToConnect=dynlen( systems )+1;  //IM 81040
  }
  else
  {
  ...
  connectedSys=aes_countConnectedSystems( g_counterConnectId, g_closedIdf );

  //aes_debug( _func_+"() / g_CounterConnectId=" + g_counterConnectId, 10 );
  //aes_debug( _func_+"() / g_closedIdf=" + g_closedIdf, 10 );
  //aes_debug( _func_+"() / connectedSys=" + connectedSys + " sysToConnect=" + sysToConnect, 10 );
  ...
  for( i=1; i <= l; i++ )
  {
    aes_getInfoFromIdentifier( g_counterConnectId[i], tabType, mode, sysId, fromProp ); 
  ...
  //aes_debug( _func_+"() / ClosedIdfList=" + g_closedIdf, 10 );
  for( i=1; i <= l; i++ )
  {
...
void aes_initBusy()
{
  const string _func_="aes_initBusy";
  const int d=5;
  ...
  {
    //��� dialog
    aes_debug( _func_+"() / dpConnect failed !", d );
  }
}
...
{
  const string _func_="aes_initDistInfo";
  const int d=5;
...
aes_workCB(string ident, dyn_dyn_anytype tab, bool fromDpQuery=false )
{
  ...
  const string    _func_="aes_workCB";
  const int d=5;
  ...
  // if we reach this position without leaving function, we try to append the system id / only if we are not fromProp ( means no dpQuery data's / closed list )
  if( !fromProp )
  {
    if( mode != AES_MODE_CLOSED )
      startThread( "aes_appendIdent4SysId", systemId );                 //// sysid aus Daten ??????????????????
  }
  // abort if we have only the header information / no datas
  // exception - if we are in checkAll mode we have to go on and check lastError
  // possible we lost a system connection - we want get datas in this case only
  // a hl with an error
...
doTimeStamp("AFTER CONVERTALERTTAB" );
  ...
  aes_debug( _func_+"() - after convertAlertTab xtablen=" + dynlen(xtab) + " xtab2len=" + dynlen(xtab2), d );
  if ( (dynlen(xtab2) != 0) && (dynlen(xtab2[1]) != 0) )
  {
    bLinesDeleted = TRUE;
    if ( mode == AES_MODE_CURRENT )
    {
      aes_debug( _func_+"() - DELETE cmd=" + g_tableCmdDelete );
  ...
  if ( nCount != 0 )
  {
    if (    mode == AES_MODE_CLOSEDAPP ||
         (  mode == AES_MODE_CLOSED && screenType == AESTYPE_EVENTS ) )
    {
      ...
      aes_debug( _func_+"() - APPEND cmd=" + g_tableCmdAppend, d );
    ...
    else
    {
      ...
      aes_debug( _func_+"() - UPDATE cmd=" + g_tableCmdUpdate, d );
      ...
      else
      {
        ...
        else
        {
          ...
          for(int i=1;i<=dynlen(xtab[4]);i++){
            /*
            strreplace(xtab[4][i],"SysGrabosjec","Vis");
            //strreplace(xtab[4][i],"SysDrivusa", "Test");
            strreplace(xtab[4][i],"SysSuhodol","25.Novembar-Grab");
            strreplace(xtab[4][i],"SysGaj","Gaj-Igman");
            strreplace(xtab[4][i],"SysBijelaVlaka","BijelaVlaka");
            strreplace(xtab[4][i],"System1","Obilaznica");
            strreplace(xtab[4][i],"SysVijenac","1.Mart");
            */
           }
...
doTimeStamp("AFTER DISPLAY DATA" );
  ...
  if( mode == AES_MODE_CLOSEDAPP || screenType == AESTYPE_EVENTS)   // nur bei alerts/closed - if f�r events nicht notwendig
  {
    aes_getSortList( g_propDp, sortList, sortAsc );
  ...
  getValue("", "lineCount", iNewLineCount); 
    ...
    if ( iNewLineCount != iLineCount || bLinesDeleted )                       // new information !!
    {
      ...
      {
        if(( iLineCount-1 ) == lastRow ) //if previously showing the last line, move to show whichever is the last line now
          this.lineVisible = -1;
        else if(firstRow == lastRow) //if previously showing less than one full table, then move to show the last line
          this.lineVisible = -1;
      }
      else
      {
  ...
  // necessary for linux ui / last line visible in table - �im60549
  //this.lineVisible=-1;    // fix at table for linux necessary
}
...
{
  int lineVis;
  const string _func_="aes_getARLineVis";
...
{
  const string _func_="aes_updateLineCount";
  int count, lineVis;
  string pre, app, tmp;
  ...
  if( g_alertRow )
  {
    ...
    //sanjin - bio exception u logovima
    /*
...
void aes_arVisible( bool up=true )
{
  ...
  const string _func_="aes_arVisible";
...
{
  const string _func_="aes_doDpQuery";
  const int d=0;
...
/////// neu
  ...
  aes_debug(_func_+"() / HISTDATA TREATING start=" + st1 + " end=" + st2, d );
  ...
  if( valCheckAll )
  {
    ...
    {
      //��� dialog
      aes_debug(_func_+"() / No systems found ( for open mode history ) !", d );
      return false;
    }
  }
  ...
  aes_debug(_func_+"() / checkall=" + valCheckAll + " systems=" + valSystemSelections + " len=" +iNumberSystemSelections, d );
  ...
  aes_debug(_func_+"() / IN SYSTEM LOOP g_counterConnectId=" + g_counterConnectId + " iSystemId=" + iSystemId + " idf=" + idf );
      ...
      if( screenType == AESTYPE_EVENTS )
      {
        ...
        // orig tmp =  "SELECT FROM " + dsFrom[i] + remoteSelect + " WHERE " + where +
        tmp =  "SELECT FROM " + dsFrom[i] + remoteSelect + whereString +
                 " TIMERANGE(\"" + st1 + "\",\"" + st2 + "\",1,0) "+sFilterLang;
        select =  "SELECT " + dsAttrList[i] + " FROM " + dsFrom[i] + remoteSelect + whereString +
                 " TIMERANGE(\"" + st1 + "\",\"" + st2 + "\",1,0) "+sFilterLang;
      }
      else
      {
        // orig tmp =  "SELECT ALERT FROM " + dsFrom[i] + remoteSelect + " WHERE " + where +
        tmp =  "SELECT ALERT FROM " + dsFrom[i] + remoteSelect + whereString +
                 " TIMERANGE(\"" + st1 + "\",\"" + st2 + "\",1,0) "+sFilterLang;
        select =  "SELECT ALERT " + dsAttrList[i] + " FROM " + dsFrom[i] + remoteSelect + whereString +
                 " TIMERANGE(\"" + st1 + "\",\"" + st2 + "\",1,0) "+sFilterLang;
      }

      //aes_debug(_func_+"() / DPQuery=" + tmp, 10 );
      int iQuerySuccess = dpQuery( select, tab );
      ...
      if( iQuerySuccess == -1 )
      {
        if( lastSystem )
          aes_stopBusy( g_tabType );
//IM 109935: Die Ergebnisse der erfolgreichen Abfragen d�rfen nicht gel�scht werden nur weil eine andere Abfrage schief geht

        continue;
//       if( aes_checkQueryError( err, iSystemId, 2 ) == AES_CHKERR_ABORT )
//       {
//         this.deleteAllLines;
//         aes_doStop( g_propDp );
//         return false;
...
//dspitzer: ENDE
      ...
      //aes_debug( _func_+"() / dpQUery len=" + dynlen( tab ) + " ====================================>" );
...
//dspitzer: ENDE
      ...
      //                 ( sc.cat - toomuchalert3 (replace "�" by 'g_asMaxLinesToDisplay')
      //              -> Query ready (End time reached): "Question: Display?"
      //                 ( sc.cat - toomuchalert2 (replace "�" by 'g_asMaxLinesToDisplay',
      //                                           "�" by 'lines')
  ...
  //*****************!!!
      ...
      if( screenType == AESTYPE_EVENTS )
      {
        identifier=aes_createIdentifier( g_tabType, AES_MODE_CLOSED, iSystemId, true );      // nur dummy, mu� in workCB abgefangen werden damit connectliste nicht erweitert wird
      }
      else
      {
  ...
  if ( ! valChrono && (action != AES_ACTION_SAVE ) )
  {
      ...
      aes_debug( _func_+"() / CALL WORK ident=" + identifier + " ==> !valchrono/action !Save ", d );      
...
{
  const string _func_="aes_deleteSystemTableRows";
  const int d=0;

  //aes_debug(_func_+"() / DELETING datas for system=" + sysName, d );
...
)
{
    ...
    if( dpQueryConnectSingle( AESQUERY_WORKFUNC, wantsAnswer, identifier, command, g_queryBlockedTime ) == -1 )
    {
    ...
    if(valCheckAllSystems)//IM 98673 checks if showAllSystems is selected
    {
      dyn_uint duSysId;
      dyn_string dsSystemNames;
      getSystemNamesConnected(dsSystemNames, duSysId); //get All connected systems
      for(int i = 1; i <= dynlen(duSysId); i++)
      {
        identifier=aes_createIdentifier( g_tabType, valType, duSysId[i] ); //create an Identifier for all connected systems
        aes_appendIdentifier( identifier );
      }
    }
}
...
{
  const string _func_="aes_doQueryConnect";
  const int d=5; // never used!!!!
  ...
  for( iCountSystems = 1; iCountSystems <= iNumberSystemSelections; iCountSystems ++ )
  {
    ...
    if( screenType == AESTYPE_EVENTS )
    {
      ...
      command="SELECT " + attrList + " FROM " + from + remoteCommand + whereString; 
      tmpCmd="SELECT attrList FROM " + from + remoteCommand + whereString;
    }
    else
    {
      // orig command="SELECT ALERT " + attrList + " FROM " + from + remoteCommand + " WHERE " + c_where;
      command="SELECT ALERT " + attrList + " FROM " + from + remoteCommand + whereString; 
      tmpCmd="SELECT ALERT attrList FROM " + from + remoteCommand + whereString; 
    }
    ...
    if( valCheckAllSystems )
    {
      // in check all mode we have to use the dummysysid because we don't know the rigth one
      iSystemId=AES_DUMMYSYSID;
    }

    startThread("aes_dpQueryConnectThread", valType, valSystemSelections[iCountSystems],
                  valCheckAllSystems, wantsAnswer, iSystemId, command);    
...
//             bool valTypeAlertSummary,
             ...
             bit32  valUserbits,
             //------------ neu
             //unsigned valFSState, - oben siehe valState
  ...
  const string _func_="aes_propertiesScreenCB";
  const int d=5;
...
aes_debug( _func_+"() / checkAllSystems=" + valCheckAllSystems );
...
//2 �berparametrierung der Spaltensichtbarkeit aus Properties beruecksichtigen !!!


///////// PR�FEN OB HIER NOCH NOTWENDIG !!!!!!!!!!!!!!!!!!!!!
...
//   std_startBusy();
  ...
  */
  ...
  else
  {
    ...
    valOpenClosedMode,
    valDirection,
    valLogicalCombine,"", strMinPrio, valSystemSelections, valCheckAllSystems
    );
  ...
  if ( valType != AES_MODE_CURRENT )   // open mode or closed mode / even events !!!
  {
    ...
    if (noOfDpes == -1 || noOfDpes > g_asMaxDpeToDisplay)
    {
      ...
      // Es ist m�glich, da� die Anzahl sehr gro� ist, und damit die Abfrage lange dauert.
...
{
  const string _func_="aes_asGetFromWhere";
  const int d=5;
  int n;
  int ic=0;
...
/////////////��� ACHTUNG - siehe Properties/Pr�fe vierten Operator !!!!!!!!!!!
/////////////////////////////////////////////////////////// Meldetext
  ...
  //��� pr�fe "IN RANGE" => prio wurde nach zahl ge�ndert / keine Bereichsangaben mehr m�glich / ACHTUNG string !
...
//aes_debug( _func_+"() / valPrio=" + valPrio + " where=" + where, 6 );
...
{
  const string _func_="aes_getQueryGroupClause";
  const int d=0;
  string res="", groupName, dpeName;
  dyn_string dsGroup, dsDpe;
  int n,l;
  bool group=false;
  ...
  // IM 106914, 109190
  int i, j, k, m, overflow = 0;
  string rdbRes, groupResolved;
  dyn_uint duiSystemIds;
  dyn_string groupNameDps, groupNameTypes, dsItemsInFilter, dsDpsList, dsSplitDpsList, dsSystemNames;
  paCfgReadValue(PROJ_PATH+CONFIG_REL_PATH+"config","ui","queryRDBdirect",rdbRes);
  ...
  //systems
  for(int z = 1; z <= dynlen(dsSelectedSystems); z++)
  {
    dynClear(groupNameDps);
    dynClear(groupNameTypes);
    // groups
    l=dynlen( dsGroup );
    if( l > 0 )
    {
      ...
      for( n=1; n<=l; n++ )
      {
        ...
        groupName=dsSelectedSystems[z]+":"+dpSubStr( groupName, DPSUB_DP_EL );
        // IM 106914, 109190
        if ( rdbRes == 1 )
        {
          dsDpsList = makeDynString();
          groupGetFilterItems(groupName, groupNameTypes, groupNameDps);
          
          for(int t = 1; t <= dynlen(groupNameDps); t++)
            groupNameDps[t] = (groupNameDps[t] != "")?dsSelectedSystems[z]+":"+groupNameDps[t]:"";

          /*for ( k = 1; k <= dynlen(groupNameDps); k++ )
          {
            dsSplitDpsList = strsplit(groupNameDps[k], ":");
            if ( dynlen(dsSplitDpsList) > 1 )
              groupNameDps[k] = dsSplitDpsList[2];
          }*/
        
          for ( i = 1; i <= dynlen(groupNameTypes); i++ )
          {
            // reading DP(E)-Items
            dsItemsInFilter=makeDynString();

            groupGetFilteredDps(groupNameTypes[i]!=""?dsSelectedSystems[z]+":"+groupNameTypes[i]:"", groupNameDps[i], dsItemsInFilter, overflow);
          ...
          /*for ( m = 1; m <= dynlen(dsDpsList); m++ )
            dsDpsList[m] = dpSubStr(dsDpsList[m], DPSUB_DP);*/
        
          groupResolved = dsDpsList;
          strreplace(groupResolved, " | ", ",");        
          res += ( (strlen(res) < 3 || res[strlen(res)-1]==",")?"":"," ) + groupResolved;
        }
        else
        {          
  ...
  //aes_debug(_func_+"() / valDpList="+dpList, d );
  //aes_debug(_func_+"() / res=" + res, d );
...
{
  const string _func_="aes_getTableColumnName";
  int l, n;
  int screenType;
  bool visCol;
  dyn_dyn_anytype ddaCol;
...
{
  const string _func_="aes_getTableIndex";
  int l, n;
  bool visCol;
  string name;
  dyn_dyn_anytype ddaCol;
...
{
  const string _func_="aes_getVisibleColumnList";
  const int d=5;
  int count, j, i, width;
  bool vis;
  dyn_string tcolNames, tcolTitles;
  ...
  aes_debug( _func_ + "() / screenType=" + screenType, d );
...
//DebugN(_func_+"() / tcolNames before=" + tcolNames );
...
{
  const string _func_="aes_createConfigMatrix";
  int i, n, l, ic=0;
  int c=CFG_LAST;
...
//  dynClear( configMatrix );

  //attributes=g_attrList;
...
/*            
            if( onClick != AECSF_NONE )
            {
              if( onClick == AECSF_ACKNOWLEDGE || onClick == AECSF_INSERTCOMMENT )
              {
                // for new treating we can use it directly
                //aVal=aes_getPureFunctionName( onClick );
                aVal=onClick;
              }
              break;
            }
            ...
            {
              // we need only the functionname without parenthesis
              if( func != "" )
              {
                aVal=aes_getPureFunctionName( func );
              }
              else
              {
                aVal="";
...
*/
          ...
          default:
            aes_debug( _func_+"() / Defaultstatement reached !" );
...
/**
  This function returns the settings values. 
  @param settings dyn_anytype reference 
  @author Martin Pallesch
  @version 1.0
  @return nothing
*/
void aes_createSettings( 
  ...
  string backCol,
  string foreCol,
//  dyn_langString dlsBLEComment )
  dyn_string dsBLEComment,
  dyn_string dsDPEList )
{

  const string _func_="aes_createSettings";
  bool useFontProp;
  string screenFont;
  string printoutFont;
  dyn_string dsTmp;
...
//DebugN( "createSettings(E)  =" + g_showInternalsE );
  ...
  aes_debug(_func_+"() / OneRowPerAlert=" + multiLineSuppress );
  ...
  {
    aes_debug(_func_+"() / Unkown screenType="+screenType+" !" );
  }
...
{
  const string _func_="aes_createPopUpMenu";
  const int d=0;
...
/**
  This function builds the menustring and displaies the popup menu. 
  @param none
  @author Martin Pallesch
  @version 1.0
  @return nothing
...
{
  const string _func_="aes_deleteRTDps";
  const int d=5;
  int i,l ;
  dyn_string ds, dsTmp;
  int pos;
  ...
  //aes_debug(_func_+"() / Delete List=" + ds, 10 );

  l=dynlen(ds);
  if (getUserPermission(4))
  for( i=1; i<=l; i++ )
  {
    aes_debug(_func_+"() / Delete RTDP("+i+")=" + ds[i], d );
...
void aes_panelOff( bool fromPanel=false )
{
  ...
  const string _func_="aes_panelOff";
                       ...
                       int row, int column, anytype value, mapping mTableMultipleRows )
{
  ...
  if( ( content != "" ) || g_alertRow )
  {
    ...
    aes_displayMenu( screenType, row, column, value, propDp, tableName, tabType, emptyRow, mTableMultipleRows );
  }
}
                      ...
                      const string tableName, const int tabType, bool emptyRow, mapping mTableMultipleRows )
{
  ...
  const string _func_="aes_displayMenu";
  const int d=0;
  ...
  // default visibilty settings of popup menu
  bool  title         =true,
        ack           =true,
        denyava       =false,
        insertComment =true,
...
//IM 95447: allow online alert class change in Current mode with RDB for discret alarms      
  ...
  aes_debug( _func_+"() row="+row+" column="+column+" val="+val, " propDpName=" + propDpName + " tableName=" + tableName + " tabType=" + tabType, d );
  ...
  for( n=first; n < last; n++ )
  {
    ...
    switch( entryType )
    {
      ...
      default:
       aes_message( AESMSG_GENERAL_ERROR, makeDynAnytype(), 1, _func_ );        
    }
  ...
  if( dynlen( dsMenu ) > 0 )
  {
    ...
    if( res == 0 )
    {
      aes_menuFunctionHandler( propDpName, answer, row, column, val, tabType, tableName, emptyRow, mTableMultipleRows ); 
    }
  }
...
void aes_commentAction(int row, string panel, mapping mTableRow)
{
  ...
  { 
       dsTemp = strsplit(ds[1], "\uA7");
       t= ti;
      if ( alertSet(t, getACount(ti), dpSubStr(getAIdentifier(ti), DPSUB_SYS_DP_EL_CONF_DET) + "._comment", ds[1]) == -1 )
      {
        std_error("AS", ERR_SYSTEM, PRIO_SEVERE,
...
/*
  ...
  if ( ackable && oldest )
  {
    DebugN("NEW - acknowledge" );
    //��� quittieren bei insertComment ggf hier deaktivieren
...
void aes_displayTrend(int row, mapping mTableMultipleRows)
{
  ...
  const string _func_="aes_displayTrend";
  const int d=5;
...
{
  const string _func_="aes_displaySumalertDetails";
  const int d=5;
  int mode;
...
{
  const string _func_="aes_doPropControlConnect";
  ...
  {
    aes_message( AESMSG_DPCONNECT_FAILED, makeDynString( g_propDp ), 1, _func_ );
    ret=-1;
    return;
  }
...
void aes_triggerPropCB( string &propDp, bool interval=false )
{
  ...
  {
    // set start time for historical data !!!
  ...
  // this dummy set operation triggers the propertyCB
...
{
  const string _func_="aes_getRunMode";
  const int d=0;

  //aes_debug( _func_+"() / RunMode=" + runMode + " - called from : " + dummy, d );
...
{
  const string _func_="aes_resortTab";
  const int d=5;
  int i, j, retIdx = 1;
  dyn_dyn_anytype ret;
  time t;
...
DebugN(_func_+"() / Called xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
  ...
  if( _dirIdx <= 0 )
  {
    ...
    aes_debug( _func_+"() / Needed attribute=" + AES_SEARCH_DIRECTION + " not found !", d );
    return ret;
  }
...
void aes_getDpesOfFilter(string from, bool as, int &noOfDps)
{
  ...
  //��� ACHTUNG - diese Funktion kann nicht f�r eine Kombination aus Group und Dpes verwendet werden !!!!

  const string _func_="aes_getDpesOfFilter";
  int        i, j, overflow, n;
  string     select, dpeT;
...
{
  const string _func_="aes_doRestart";
  unsigned runMode;
...
{
  const string _func_="aes_discardTimeOut";
  string pre, app;
  bool vis;
...
bool aes_checkMaxLinesToDisplayProp( int lines, int screenType )
{
  ...
  const string _func_="aes_checkMaxLinesToDisplayProp";
...
bool aes_checkMaxLinesToDisplay( int dlTab, int screenType, string ident )
{
  ...
  const string _func_="aes_checkMaxLinesToDisplay";
  dyn_float  df;
  dyn_string ds;
  string     sTemp;
...
int aes_checkQueryError( dyn_errClass err, int iSystemId, int id=-1, bool fromDpQuery=false )
{
  ...
  const string _func_="aes_checkQueryError";
  ...
  aes_debug( _func_+"() / ERROR called from=" + id + " sysId=" + iSystemId + " errCode=" + errCode + " fromDpQuery=" + fromDpQuery, 0 );

  //DebugN(_func_+"() / errCode=" + errCode ); 
  //DebugN(_func_+"() / err=", err[1] ); 
...
//**** new test -> from reduDistTreating
...
{
  const string _func_="aes_as_tableToFile";
  int        iCount;
  string     sFile;
  ...
  else
  {
    ...
    if( dynlen( dsFileNames ) > 0 )
    {
      ...
      {
        // Overwrite file
        setValue( tableName, "writeToFile", sFile, false, "\t" );
      }
    }
    else
    {
...
{
  const string _func_="aes_doStart";
  unsigned runMode;
  int n=1;
...
/*
  ...
  @return nothing
*/
void aes_removeIdent4SysId( int remSysId )
{
  const string _func_="aes_removeIdent4SysId";
  int n,l;
  ...
  for( n=l; n > 0; n-- )
  {
      ...
      ( lostAllCon && ( systemId != mySystemId && systemId != AES_DUMMYSYSID ) ) )
    {
      ...
      aes_deleteSystemTableRows( (string)getSystemName( systemId ) );
...
{
  const string _func_="aes_appendClosedIdent4SysId";
  string closedIdf;
  ...
  {
    //aes_debug( _func_+"() / Append closedIdf=" + closedIdf );
...
{
  const string _func_="aes_appendIdent4SysId";
  const int d=5;
  ...
  {
    aes_debug( _func_+"() Append ident=" + identifier, d );
    aes_appendIdentifier ( identifier );     
    aes_createDistInfo();
  }
  return true;
  ...
  string dpChanged,       unsigned  valChanged )
{
  ...
  const string _func_="aes_propControlCB";
  const int d=0;
...
//aes_debug(_func_+"() / valChanged=" + valChanged, 10 );
  ...
  {
    // es wird nicht explizit ausgef�hrt - nur �ber zwei screenType changes
  }
...
//��� auch cancel abfangen !!!!!
  ...
  if( valChanged == AES_CHANGED_RUNCMD )
  {
    ...
    aes_getRunMode( g_propDp, runMode, _func_ );
...
{
  const string _func_="aes_runCmdStart";
  const int d=0;
...
{
  const string _func_="aes_runCmdStop";
  const int d=0;
  string workDpName;
  string pre, app;


  //aes_debug(_func_+"() propDpName=", 5 );
  if( propDpName == "" )
...
{
  const string _func_="aes_runCmdPause";
  const int d=0;
  bool tableRunning;
...
{
  const string _func_="aes_getStatusString";
  const int d=0;
  ...
  aes_getRunMode( propDp, runMode, _func_ );

//aes_debug(_func_+"() / runMode=" + runMode, 5 );
...
{
  const string _func_="aes_getSettings";
  const int d=0;
...
{
  const string _func_="aes_displayStatus";
  const int d=0;
  bool paused;
...
//aes_debug(_func_+"() / tab=" + headerText + " textBox=" + sRunMode, 5 );
...
{
  const string _func_="aes_treatOldConfigs";
  const int d=0;
...
{
  //��� ACHTUNG - problem aus altem as betr. finde �ltesten alarm !!!
  string propDp;
...
void aes_changedPropConfig(bool bChangeAesType=false)
{
  ...
  const string _func_="aes_changedPropConfig";
  const int d=0;
  ...
  aes_debug(_func_+" tabType="+tabType + " activeReg=" + activeReg + " mySct=" + mySct + " balSct=" + balSct, d );
...
{
  const string _func_="aes_changedRegTab";
  const int d=5;
  aes_debug( _func_, d );
...
{
  const string _func_="aes_changedScreenType";
  const int d=0;
  ...
  aes_debug(_func_+"() - tabType="+tabType+" screenType="+screenType+" name="+this.name );
...
void aes_propertyDialog( int tabType, bool fromGeneral=false, bool alertRow=false, bool configPanel=false )
{
  ...
  // if genereal g_propDpNameTop if not g_propDpName

  
  // code from general !!!
  const string _func_="aes_propertyDialog";
  const int d=5;
  shape sConfig;
...
{
  const string _func_="aes_changedActivity";
  ...
  //aes_debug( _func_+"() / tabType=" + tabType + " actTop=" + activeTop + " actBot=" + activeBot );


//aes_debug( _func_+"() / aTop=" + activeTop + " aBot=" + activeBot, 5 );
...
void aes_doPropertiesConnect( int &ret )
{
  ...
  const string _func_="aes_doPropertiesConnect";
...
//dpGet        "_Config.MinPrio" + AES_ONLVAL,
        ...
        // new
//dpGet        "_Config.ShowInternals" + AES_ONLVAL    /// aus as_initOld raus - hier damit onlinechange m�glich
                                                               ...
                                                               /////// dpget sinvoller ??????????
{
  ...
  const string _func_="aes_propertiesCB";
  const int d=5;
  ...
  aes_debug( _func_+"() / valCheckAllSystems=" + valCheckAllSystems );
  ...
  //aes_debug(_func_+"() / PROPERTIESCB !!!!!!!!!", 5 );

  // we use dpGet instead of the properties hotlink to suppress hotlink
  
dpGetCache(  "_Config.MinPrio" + AES_ONLVAL, valMinPrio,
        //"_Config.ShowInternals" + AES_ONLVAL, valShowInternals,    /// aus as_initOld raus - hier damit onlinechange m�glich
...
/*
DebugN(_func_+"() / HistData=" + valHistoricalData );
DebugN(_func_+"() / KamGing =" + valCameWentSort );
DebugN(_func_+"() / modakt  =" + valFSState );
DebugN(_func_+"() / oneline =" + valOneRowPerAlert );
DebugN(_func_+"() / modopcl =" + valOpenClosedMode );
DebugN(_func_+"() / dir     =" + valDirection );
DebugN(_func_+"() / logicomb=" + valLogicalCombine );
DebugN(_func_+"() / sumalert=" + valAlertSummary );

DebugN(_func_+"() / internalDps      =" + g_showInternals );
DebugN(_func_+"() / maxClosedLines   =" + g_maxClosedLines );
DebugN(_func_+"() / minAlertPrio     =" + valMinPrio );
DebugN(_func_+"() / queryBlockTime   =" + g_queryBlockedTime );
DebugN(_func_+"() / historicalData   =" + valHistoricalData );
DebugN(_func_+"() / historicalDataInt=" + g_historicalDataInterval );
*/
...
doTimeStamp( "PROPSTART", true );
  ...
  aes_debug( _func_+"() / tabType="+g_tabType + " propMode=" + valType + " screenType=" + screenType );
...
aes_debug(_func_+"+++++++++++++++++++++++++++++ attrlist="+g_attrList );
...
void aes_initMainTable( const int tabType )
{
  ...
  const string _func_="aes_initMainTable";
  const int d=0;
  int ret;
  unsigned changed;
  ...
  aes_debug(_func_+" screenType=" + screenType +" tableName=" + tableName + " g_propDp="+g_propDp+" g_propDpNameTop="+g_propDpNameTop, d );
  ...
  aes_debug( _func_+"() / tabType=" + tabType + " g_alertRow=" + g_alertRow + " action=" + action + " active=" + active, d );
  ...
  if( g_alertRow || ( action != AES_ACTION_INTERACT ) )
  {
    ...
    if( active )
    {
      ...
      //aes_debug(_func_+"() / Start tabType="+tabType,5);
    }
  }
}
...
void aes_initMainReg( string screenConfig="", int action=AES_ACTION_INTERACT )
{
  ...
  const string _func_="aes_initMainReg";
  string fileName;
  bool ret, dispSlider;
...
{
  const string _func_="aes_checkInitConditions";
  const int d=5;
  ...
  {
    //���
    aes_debug(_func_+"() / Some datatypes are missing - abort program !", d );
    return;
  }
  ...
  {
    //���
    aes_debug(_func_+"() / Config dp is missing - abort program !", d );
    return;
  }
...
void aes_initOldScreenVariables()
{
  ...
  ES_TYPEFILTER = makeDynString( "bit", "bit32", "unsigned integer", "integer", 
                                 "float", "time", "string", "langstring", "dp identifier",
                                 "dyn bit", "dyn bit32", "dyn integer", 
                                 "dyn unsigned integer", "dyn float", "dyn time", "dyn string",
...
{
  const string _func_="aes_addPropertyDp";
  ...
  if( !dpExists(propDp) )
  {
    aes_message( AESMSG_DPCREATE_FAILED, makeDynString( propDp ), 1, _func_ );
    return -1;
  }
...
string aes_getPropDpName( const int dpType, bool runtime=false, int tabType=AESTAB_TOP, bool fromAROut=false, bool configPanel=false, bool localConfig=false )
{
  ...
  dyn_string names;
  int count,pos;
  string _func_ = "aes_getPropDpName";
...
/**
  Main registercard initalisation routine. 
  @param dpProp name of DP on which to set the values (must be of DP-Type _ASConfig); with trailing "."
  @author Martin Pallesch
  @version 1.0
  @return nothing
*/
...
/**
  This funcion. 
  @param queryCommand AES_QUERYCMD_* ( START / STOP / PAUSE )
  @author Martin Pallesch
  @version 1.0
  @return nothing
*/
int aes_queryDisconnect( dyn_string &identifiers )
{
  ...
  const string _func_="aes_queryDisconnect";
  const int d=5;
  int n, l;
  int errFlag=false;
  string identifier;
  ...
  for( n=1; n <= l; n++ )
  {
    ...
    {
      //aes_debug( _func_+"() / Disconnect="+identifier, d );
    }
    else
    {
      errFlag=true;
      aes_debug( _func_+"() / Error during disconnect for system="+identifiers[n], d );
    }
  }
...
/////***************************************************
/////***************************************************
///// RegisterCard function block !!!! new

/**
  This funcion. 
  @param queryCommand QUERYCMD_* ( START / STOP / PAUSE )
  @author Martin Pallesch
  @version 1.0
  @return nothing
*/
dyn_string aes_resortConfigs( const dyn_string &realNames )
{
  ...
  const string _func_="aes_resortConfigs";
  int pos, n, ic=1;
  string val;
...
{
  const string _func_="aes_treatActivityChange";
...
{
  const string _func_="aes_treatScreenConfigChange";
  const int d=0;
  ...
  if( screenConfigName != "" )
  {
    ...
    if( scDpName == "" )
    {
      aes_debug( _func_+"() no dp for name=" + screenConfigName );
      return;
    }
  }
  else
  ...
  aes_debug(_func_+"() tabType="+tabType+" screenConfigName="+screenConfigName+" dp="+scDpName, d );
  ...
  //aes_debug(_func_+"() / config="+config+" tabType="+tabType +" pre=" + pre, 5 );
  setValue( pre + "cb_config", "text", config );
  ...
  aes_debug(_func_+"() tabType="+tabType+" screenConfigName="+screenConfigName+" dp="+scDpName + " screenType=" + screenType, d );
...
{
  const string _func_="aes_treatPropConfigChange";
  unsigned screenType;
  string dpSource;
...
bool aes_operationPermission( const string configName, const int type, const int operation )
{
  ...
  if( !AES_PROTECTION_ON ) return true;
...
{
  const string _func_="aes_addScreenConfig";
  const int d=0;
  ...
  if( dynContains( configs, configName ) == 0 )
  {
    ...
    aes_debug(_func_ + "() screenConfigDp="+screenConfigDp, d );
    ...
    if( !dpExists(screenConfigDp) )
    {
       aes_message( AESMSG_DPCREATE_FAILED, makeDynString( screenConfigDp ), 1, _func_ );
       ret=-1;
       return ;
    }
  }
...
void aes_saveScreenConfig( const string &screenConfig, int &ret )
{
  ...
  const string _func_="aes_saveScreenConfig";
  const int d=0;
  ...
  if( scDp == "" )
  {
    aes_debug(_func_+"() Empty screenconfigdatapointname !", d );
    ret=-1;
    return;
  }
  ...
  {
    aes_debug(_func_+"() dpSet for dp="+scDp+" failed !", d );
    ret=-1;
    return;
  }
...
{
  const string _func_="aes_removeSreenConfig";
  const int d=0;
  string scDp;
  dyn_string configs;
  int pos;
  ...
  if( scDp != "" )
  {
    if( getUserPermission(4) )
    {
      ...
      if ( dpDelete( scDp ) != 0 )
      {
        aes_debug(_func_+"() dpDelete for dp="+scDp+" failed !", 10 ); 
        ret=-1;
      }
    }
  }
  ...
  {
    aes_debug(_func_+"() Empty dp name !", d );
    ret=-1;
  }
...
{
  const string _func_="aes_renameSreenConfig";
...
{
  const string _func_="aes_copyProperties";
  const int d=5;
...
//    dpTarget + ".Alerts.FilterTypes.AlertSummary"+AES_ORIVAL, fiAlertSummary,
    ...
    {
      aes_debug(_func_+"() dpSetWait() failed !" );
    }
...
void aes_getDpsPerType( dyn_string &realNames, dyn_string &dps, dyn_bool &dbdef, int dpType, int screenType=AESTYPE_GENERAL, bool runtime=false, int alertRowMode=AES_ARPROP_EXCL )
{
  ...
  const string _func_="aes_getDpsPerType";
  const string APP="_*";
  const string RT="RT";
...
// DebugTN("$$$ "+_func_+"() / dpType=" + dpType + " screenType=" + screenType + " RT=" + runtime + " arMode=" + alertRowMode );
  ...
  {
    aes_debug( _func_+"() / Unkown dpType="+dpType, 1 );
  }
...
//  DebugTN("$$$ "+_func_+"() STOP ("+count+" dpGets)/ dpType=" + dpType + " screenType=" + screenType + " RT=" + runtime + " arMode=" + alertRowMode, getCurrentTime()-t );
...
/*
  ...
  @return nothing
*/
//void aes_filterSLItems( const dyn_string items, dyn_string &dsResult )
void aes_filterSLItems( const dyn_string colTitles, const dyn_string colNames, dyn_string &dsTitles, dyn_string &dsNames )
{
  ...
  aes_debug( _func_+"() / Called !" );

  // check whether len matches
  if( dynlen(dsTitles) != dynlen(dsNames) )
  {
    aes_debug( _func_+"() / dynlen doesn't match !", d );
    return;
  }
...
{
  const string _func_="aes_getStringFromLangString";
  dyn_string ds;
  ...
  returnValue = (string) ls;

  if( returnValue == "" )
  {
    // fallback - try to find meta lang
    idx=getLangIdx( AES_LANG_META );
    idx++; // because index starts at zero !!!
...
/*  - old / im 60421  
    if( idx >= 0 )
    {
      return (string)ds[idx+1];
    }
    else
    {
      // if we didnt find the metaidx we havent anything to return, return english text
      return (string) ds[getLangIdx( AES_LANG_EN )] ;
    }
*/
  }
  else
...
{
  const string _func_="aes_setProps";
...
//  if ( tiType != AES_MODE_CURRENT ) fiState = AES_MODECUR_ALL;  // in open or closed protocoll, we want all states
  ...
  if( !dpExists(dpCheck) )
  {
     aes_message( AESMSG_DPCREATE_FAILED, makeDynString( dpCheck ), 1, _func_ );
     ret=-1;
     return;
  }
...
////!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  */
  ...
  if( newProperty )
  {
    ...
    setMultiValue( "vi_list", "items", g_colTitles,
                   "vi_columns", "items", dsEmpty );
  }

  if( tiMaxLines < AES_MIN_OPEN_LINES )
    tiMaxLines=AES_MIN_OPEN_LINES;
...
{
  const string _func_="aes_getPropsSort";
...
/*  
  ...
  if( screenType == AESTYPE_ALERTS || screenType == AESTYPE_EVENTS )
  {
    dda=ddaRes[screenType]; count = dynlen(dda);
  }
  else
  {
    return;
...
//��� ausmaskieren
  ...
  for (i = 1; i <= count; i++)
  {
    ...
    if ( !vis ) continue;
    j++;
    g_colNames[j] = dda[i][TE_NAME];
    g_colTitles[j] = dda[i][LT_HEADERNAME];
  }
//���

  // we have to filter the column items
// g_colTitles, g_colNames
  //aes_filterSLItems( colTitles, colNames, colTitles, colNames );
*/

  // we have to delete non existing items from target list ( possible configuration change )
  //aes_mergeSL( colTitles, soList, soList );
...
{
  int n, l;
  const string _func_="aes_correctVisibleColumnSortOrder";
  string col;
...
aes_getPropsVisible(string dpProp, int screenType)
{
  ...
  viList=aes_correctVisibleColumnSortOrder( viList );

  colNames=g_colNames;
  colTitles=g_colTitles;

  count=dynlen( colNames );

  // create dyn_string of column-names 
  for (i = 1; i <= count; i++)
  {
    //if ( dynContains(viList, dda[i][20]) )
    //if ( dynContains(viList, colNames[i] ) > 0 )
    if ( dynContains(viList, colNames[i] ) > 0 )
    {
      dynAppend( selected, colTitles[i] );            // g_colTitles
    }
    else
    {
      dynAppend( dsTitles, colTitles[i] );            // g_colTitles
    }
  }

  setMultiValue("vi_columns", "items", dsTitles,
...
void aes_propInit( int screenType, int tabType, string configName, string dpAESProp )
{
  ...
  if ( ! dpExists(dpAESProp) )
  {
    ...
    if( !dpExists(dpAESProp) )
    {
       aes_message( AESMSG_DPCREATE_FAILED, makeDynString( dpAESProp ), 1, _func_ );
       return;
    }
...
void aes_runBoth()
{
  ...
  const string _func_="aes_runBoth";
  bool actTop, actBot;
  ...
  if( actBot )
  {
    ...
    {
      aes_getRunMode( g_propDpNameTop, runMode, _func_ );
...
{
  const string _func_="aes_openAES";
  const int d=5;
...
{
  const string _func_="aes_setRunTimeProp";
  string propDp;
...
//     ret = dpCreate(dp, dpType);
  ...
  dpSet("_CtrlDebug_CTRL_1.Command:_original.._value", "aesGenerateDp "+dp+" "+dpType); 
...
bool aes_getFiltered_and_ForceFiltered_Attribute( string dpe, string dpDetail, time ti, int count, bool &filtered, bool &force_filtered )
{
  ...
  if (iType != DPCONFIG_SUM_ALERT )
  {
    select =  "SELECT ALERT "+
              ...
              " FROM " + "'"+dpSubStr(dpe, DPSUB_SYS_DP_EL) +"'";
  }
  else  // sum alert
  {
...
aes_connectAllDistributedSystems()
{
  ...
  DebugFTN("aesDist",__LINE__,"aes.ctl - aes_connectAllDistributedSystems",gds_connectedSystems,gds_queryConnectedSystems);
  ...
  DebugFTN("aesDist",__LINE__,"aes.ctl - aes_connectAllDistributedSystems - systems to connect",ds_systemsToConnect);
...
aes_connectNewDistributedSystems()
{
  ...
  DebugFTN("aesDist",__LINE__,"aes.ctl - aes_connectNewDistributedSystems",gds_connectedSystems,gds_queryConnectedSystems,gds_valSystemSelections);
  ...
  DebugFTN("aesDist",__LINE__,"aes.ctl - aes_connectNewDistributedSystems - systems to connect",ds_systemsToConnect);
