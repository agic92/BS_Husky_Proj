      ...
      else
      {
        ...
        else
        {
          ...
          for(int i=1;i<=dynlen(xtab[4]);i++){
            ...
            string dp=dpSubStr(xtab[4][i],DPSUB_SYS_DP);
            string objekat;
            dpGet(dp+".objekat",objekat);            
            
            strreplace(xtab[4][i],"SysGrabosjec:","Lepenica-Tarcin  / "+objekat+" / ");
            strreplace(xtab[4][i],"SysSuhodol:","Lepenica-Tarcin / "+objekat+" / ");
            strreplace(xtab[4][i],"SysGaj:","Sar.Zapad-Lepenica / "+objekat+" / ");
            strreplace(xtab[4][i],"SysBijelaVlaka:","Zvirovici-Bijaca / "+objekat+" / ");
            strreplace(xtab[4][i],"System1:","Sar.Sjever-Sar.Zapad / "+objekat+" / ");
            strreplace(xtab[4][i],"SysVijenac:","Zenica jug-Lasva / "+objekat+" / ");
           }
...
void aes_updateLineCount( const int alertCount=0 )
{
  ...
  if( g_alertRow )
  {
   ...
   //komentar setValue( "as_lineCount", "text", count );

    if( count > 0 )
    {
    //komentar  setValue(  "ar_currentLine", "text", iTopLine+1 );
    }
    else
