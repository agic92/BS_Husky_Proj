 ...
 *---------------------------------------------------------------------------------*/
bool podesiNivoAdaptivne3 (string cijev, float nivo) {
...
//   
//   dpSet("SysSavinac:rasvjeta_" + cijev + "_cijev.adaptivna.cmd.100%", vrijednosti[1]);
//   dpSet("SysSavinac:rasvjeta_" + cijev + "_cijev.adaptivna.cmd.75%", vrijednosti[2]);
//   dpSet("SysSavinac:rasvjeta_" + cijev + "_cijev.adaptivna.cmd.50%", vrijednosti[3]);
//   dpSet("SysSavinac:rasvjeta_" + cijev + "_cijev.adaptivna.cmd.25%", vrijednosti[4]);
  
   if (cijev=="lijeva") {
     bool prilazna;
     dpGet("SysSavinac:rasvjeta_lijeva_cijev.prilazna_zona.state.on", prilazna);
     level += 32*prilazna;
  }  
  
  dpSet("SysSavinac:rasvjeta_" + cijev + "_cijev.adaptivna.cmd.ukljucenje_int", level);
  
  logujPostavljanjeKomande(cijev, nivo + "%");
  
  delay(7);
  if (provjeriStatusAdaptivne3 (cijev, vrijednosti) == 0) {
    ...
    dpGet("SysSavinac:rasvjeta_" + cijev +  "_cijev.adaptivna.state.12_5%_R", a12);
    dpGet("SysSavinac:rasvjeta_" + cijev +  "_cijev.adaptivna.state.25%_R", a25);
    dpGet("SysSavinac:rasvjeta_" + cijev +  "_cijev.adaptivna.state.50%_R", a50);
    dpGet("SysSavinac:rasvjeta_" + cijev +  "_cijev.adaptivna.state.75%_R", a75);
    dpGet("SysSavinac:rasvjeta_" + cijev +  "_cijev.adaptivna.state.100%_R", a100);
  
    int dodatak = 1*a12 + 2*a25 + 4*a50 + 8*a75 + 16*a100;
    
    dpSet("SysSavinac:rasvjeta_" + cijev + "_cijev.adaptivna.cmd.ukljucenje_int", dodatak);
  }
  return false;
}
...
void podesiNivoBazne3 (string cijev, float nivo) {
  int vrijednost;
  ...
  dpSet("SysSavinac:rasvjeta_" + cijev + "_cijev.bazna.cmd.on", vrijednost);
 ...
 *---------------------------------------------------------------------------------*/
bool ukljuciPrilaznuOS23() {
  
  bool a12, a25, a50, a75, a100;
  
  dpGet("SysSavinac:rasvjeta_lijeva_cijev.adaptivna.state.12_5%_R", a12);
  dpGet("SysSavinac:rasvjeta_lijeva_cijev.adaptivna.state.25%_R", a25);
  dpGet("SysSavinac:rasvjeta_lijeva_cijev.adaptivna.state.50%_R", a50);
  dpGet("SysSavinac:rasvjeta_lijeva_cijev.adaptivna.state.75%_R", a75);
  dpGet("SysSavinac:rasvjeta_lijeva_cijev.adaptivna.state.100%_R", a100);
  ...
  dpSet("SysSavinac:rasvjeta_lijeva_cijev.adaptivna.cmd.ukljucenje_int", vrijednost);
  delay(7);
  dpGet("SysSavinac:rasvjeta_lijeva_cijev.prilazna_zona.state.on", prilazna1);
  
  if (!prilazna1) {
    dpSet("SysSavinac:rasvjeta_lijeva_cijev.adaptivna.cmd.ukljucenje_int", dodatak);
    return false;
  }
  return true;
}
//---------------------------------------------------------------------------------
bool iskljuciPrilaznuOS23() {
  bool a12, a25, a50, a75, a100;
...
/*---------------------------------------------------------------------------------*/
bool ukljuciPrilaznuOS13() {
  bool prilazna2;
  DebugTN("ukljuciPrilaznuOS1() pozvana");
  dpSet("SysSavinac:rasvjeta_desna_cijev.prilazna_zona.cmd.on", 1);
  delay(7);
  dpGet("SysSavinac:rasvjeta_desna_cijev.prilazna_zona.state.on", prilazna2);
  
  if (!prilazna2) {
    //dpSet("SysSavinac:rasvjeta_desna_cijev.prilazna_zona.cmd.on", 0);
    return false;
  }
  return true;
}
//---------------------------------------------------------------------------------
bool iskljuciPrilaznuOS13() {
  bool prilazna2;
 ...
 *                   PROVJERA GREŠAKA                                              * 
 *---------------------------------------------------------------------------------*/
dyn_string provjeriRezimSvihOrmara3() {
  ...
  for (int i=1; i<=dynlen(dps); i++) {    
    for (int j=1; j<=dynlen(tipovi_bazne); j++) {
      dpGet("SysSavinac:" + dps[i] + "." + tipovi_bazne[j] + ".upravljanje.daljinski", status);
    ...
    for (int j=1; j<=dynlen(nivoi_adaptivne); j++) {
      dpGet("SysSavinac:" + dps[i] + ".adaptivna.state." + nivoi_adaptivne[j], status);
...
/*---------------------------------------------------------------------------------*/
dyn_string provjeriRezimJednogOrmaraBazne3(string cijev) {
  dyn_string neispravni_rezimi;
  ...
  for (int i=1; i<=dynlen(nivoi_bazne); i++) {
    dpGet("SysSavinac:rasvjeta_" + cijev + "_cijev." + nivoi_bazne[i] + ".upravljanje.daljinski", status);
      if(!status) 
...
/*---------------------------------------------------------------------------------*/
dyn_string provjeriRezimJednogOrmaraAdaptivne3(string cijev) {
  dyn_string neispravni_rezimi;
  dyn_string nivoi_adaptivne = makeDynString("100%", "75%", "50%", "25%", "12_5%");
  
  bool status;
  for (int i=1; i<=dynlen(nivoi_adaptivne); i++) {
    dpGet("SysSavinac:rasvjeta_" + cijev + "_cijev.adaptivna.state." + nivoi_adaptivne[i], status);
      if(!status) 
...
/*---------------------------------------------------------------------------------*/
dyn_string provjeriRezimPrilazneZone3(int zona) {
  dyn_string neispravni_rezimi;
  string cijev = (zona==1) ? "desna" : "lijeva";
  bool status;
  dpGet("SysSavinac:rasvjeta_" + cijev + "_cijev.prilazna_zona.upravljanje.daljinski", status);
  if(!status) 
...
void izbaciGresku3(string greska) {
  dyn_float df;
  dyn_string ds;
...
//---------------------------------------------------------------------------------
int provjeriStatusAdaptivne3(string cijev, dyn_bool vrijednosti) {
  ...
  dpGet("SysSavinac:rasvjeta_" + cijev + "_cijev.adaptivna.state.12_5%_R", A12_5);
