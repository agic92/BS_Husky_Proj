...
{
  int                   i, j, iLineNr, PriorRange, actState, isSum;
  string                dpElement;
  time                  ti;
  unsigned              ranges;
  bool                  moreRanges;
  ...
  {                                                                     //Alerts with state "came" will be appended,
    ...
    PriorRange = j;
     dpGet(dpElement+":_alert_hdl."+PriorRange+"._act_state",actState);
  	...
  	dpGet(dpElement+":_alert_hdl.._summed_alerts_count", isSum);
    
  	  DebugTN("Start, ",dpElement,actState, isSum, PriorRange, ranges);
    
      if (ranges >=1) 
        moreRanges = 1;
      if ((actState == 1 || actState == 3) && dynContains(g_firstTab, dpElement) == 0) 
        dynAppend(g_firstTab, dpElement);
      string tps=substr(dpElement,11,6);

  	  DebugTN("Ubaceno, ",dpElement,actState);

      if (actState == 0 || actState == 2 || isSum >= 1 || tps=="VJ-TPS" ||tps=="VJ-SOS"/*|| actState == 3*/) 
      {
        iLineNr = dynContains(g_firstTab, dpElement);
        dynRemove(g_firstTab, iLineNr); 

  	  DebugTN("Izbaceno, ",dpElement,actState);     
      }
    }
  }
  /////////
  
  string cijev_t,objekat_t;
   int v_zona_t, s_zona_t;
 ...
 objekat_t=substr(dpElement,0,i); 
  dpGet(objekat_t+".cijev", cijev_t);
  dpGet(objekat_t+".vatrodojavna_zona", v_zona_t);
  dpGet(objekat_t+".saobracajna_zona", s_zona_t);
  ...
  bool mute,nadjen;
  nadjen=FALSE;
   string cijev,objekat,dpElement_t;
   int v_zona, s_zona;
   
  dpGet("AlertHorn.HornAcknowledge:_original.._value",mute);
  if(mute && dynlen(g_firstTab)>1){
  for (i = 1; i <= dynlen(g_firstTab); i++ )                                   //This section creates the alert list.
  {  
      
    //Alerts with state "came" will be appended,
    dpElement_t = dpSubStr(g_firstTab[i], DPSUB_DP_EL);                       //others as "acknowledged" will be deleted.
    if (dpElement!=dpElement_t){
      DebugN("Uso u petlju");
 
      int k;
  DebugN("dpElement_t          ",dpElement_t);
 while(dpElement_t[k]!=".")
 {
   k++;
 }
 objekat=substr(dpElement_t,0,k); 
  dpGet(objekat+".cijev", cijev);
   dpGet(objekat+".vatrodojavna_zona", v_zona);
  dpGet(objekat+".saobracajna_zona", s_zona);
  
  if(cijev==cijev_t && v_zona==v_zona_t && s_zona==s_zona_t){
     nadjen=TRUE;
     DebugN("postavio nadjen na true");
   }
  }
  }
  if(!nadjen){
...
startAlarmSound(dyn_string g_firstTab)
{
  ...
  int             beepDuration, frequency, pause_time, i, iPriority, wavLoop, ElementPrior, PriorRange, j;
