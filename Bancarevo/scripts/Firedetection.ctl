    ...
    case "bstelecom.edp.scada.IScada.reportStatus" :
        ...
        for (int i = 1; i <= dynlen(objectTypes); i++)
        {
          if (objectType == objectTypes[i][1])
          {
            ...
            getDP(systemName + "_mp_FireDetectorsSettings.settings." + objectTypes[i][1], objectStatuses);
...
bool loadSettings()
{
  ...
  if(!getDP(systemName + "_mp_FireDetectorsSettings.settings.objectTypesMeaning",
  ...
  if(!getDP(systemName + "_mp_FireDetectorsSettings.settings.detectorsObjectStatus",
   settings["DetectorsObjectStatus"])) return false;
  if(!getDP(systemName + "_mp_FireDetectorsSettings.settings.controlUnitObjectStatus",
   settings["PanelsObjectStatus"])) return false;
  if(!getDP(systemName + "_mp_FireDetectorsSettings.settings.loopObjectStatus",
